...
import { sampleProducts } from "./Product/sampleProduct"
...
const heroSlides: HeroSlide[] = [
  ...
  {
    video: "/assets/videos/Advert1.mp4",
...
export const HeroSection: FC<HeroSectionProps> = ({ onDiscoverClick }) => {
  ...
  const handleWheel = useCallback(
    ...
    [canScroll, currentSlide, handleNextSlide, handlePrevSlide],
  )
      ...
      {heroSlides.map((slide, index) => (
        ...
        >
          <video
            ...
            muted={isMuted}
            playsInline
            aria-label={`${slide.product.name} video`}
            className="absolute inset-0 object-cover w-full h-full scale-105"
          >
            <track kind="captions" srcLang="fr" label={slide.product.name} />
            Votre navigateur ne prend pas en charge la lecture de vidéos.
          </video>
        </div>
      ))}
