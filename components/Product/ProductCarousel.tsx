...
export function ProductCarousel({ products }: ProductCarouselProps) {
  ...
  const carouselRef = useRef<HTMLDivElement>(null);
  const [isAnimating, setIsAnimating] = useState(false);
  const [visibleProducts, setVisibleProducts] = useState(4); // Default to 4 products
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    setIsDragging(true);
    setStartX(e.pageX - (carouselRef.current?.offsetLeft || 0));
    setScrollLeft(currentIndex * (100 / visibleProducts));
  };
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const x = e.pageX - (carouselRef.current?.offsetLeft || 0);
    const walk = (x - startX) * 1.5;
    const newIndex = Math.round((scrollLeft - walk) / (100 / visibleProducts));
    if (newIndex !== currentIndex) {
      setCurrentIndex(Math.max(0, Math.min(newIndex, filteredProducts.length - visibleProducts)));
    }
  };
  ...
  const nextSlide = () => {
    if (isAnimating) return;
    setIsAnimating(true);
    setCurrentIndex((prev) => Math.min(prev + 1, filteredProducts.length - visibleProducts));
    setTimeout(() => setIsAnimating(false), 500);
  };
  ...
  useEffect(() => {
    ...
    if (!isHovered && !isDragging) {
      interval = setInterval(() => {
        setCurrentIndex((prev) =>
          prev >= filteredProducts.length - visibleProducts ? 0 : prev + 1
        );
      }, 5000);
    }
    return () => clearInterval(interval);
  }, [isHovered, isDragging, filteredProducts.length, visibleProducts]);
        ...
        >
          ...
          >
            {filteredProducts.map((product) => (
              <div
                key={product.id}
                className={`min-w-[${100 / visibleProducts}%] px-4`}
              >
                <a
                  href={`/products/${product.id}`}
                  className="group block"
          ...
          <button
            onClick={nextSlide}
            disabled={currentIndex >= filteredProducts.length - visibleProducts}
            className={`absolute right-0 top-1/3 -translate-y-1/2 bg-white/95 backdrop-blur-sm p-4 rounded-l-2xl shadow-lg
              transition-all duration-300 hover:bg-black hover:text-white
              ${currentIndex >= filteredProducts.length - visibleProducts ? "opacity-0 pointer-events-none" : "opacity-90 hover:opacity-100"}`}
            aria-label="Next slide"
          >
            <ArrowRight className="w-6 h-6" />
          </button>
          ...
          <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 flex items-center space-x-3">
            {Array.from({ length: Math.ceil(filteredProducts.length / visibleProducts) }).map((_, index) => (
              <button
                key={index}
